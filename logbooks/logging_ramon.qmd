...
```{r}
config <- config::get(file = "/students/2025-2026/ros1_transcriptomics/minor_ROS1/configs/config.yaml")
```
...
rownames(df) <- gene_info_unique
```

```{r}
rownames(df) <- make.unique(gene_info$hgnc_symbol, sep = "_")
```
