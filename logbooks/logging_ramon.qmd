...
gene_info_unique <- gene_info %>%
  ...
  summarise(
    hgnc_symbol = paste(unique(hgnc_symbol[!is.na(hgnc_symbol) & hgnc_symbol != ""]), 
                       collapse = "/"),
    external_gene_name = na.omit(external_gene_name)[1]
  ) %>%
  ungroup()
...
data_hg38 <- read.csv2(paste0(config$counts_dir, "/hg38_counts.csv"), sep = ",", row.names = 1)
...
```{r}
data_hg38 <- data_hg38[, rownames(col_data)]
all(rownames(col_data) == colnames(data_hg38))
```
...
for (i in 1:nrow(col_data)) {
  idx <- grep(col_data$Run[i], names(data_hg38))
  names(data_hg38)[idx] <- col_data$condition[i]
}
```
...
```{r}
HCC78_crizo <- grep("HCC78_250 nM crizotinib", names(data_hg38))
HCC78_DMSO <- grep("HCC78_DMSO", names(data_hg38))

CUTO38_crizo <- grep("CUTO38_250 nM crizotinib", names(data_hg38))
CUTO38_DMSO <- grep("CUTO38_DMSO", names(data_hg38))

CUTO37_crizo <- grep("CUTO37_250 nM crizotinib", names(data_hg38))
CUTO37_DMSO <- grep("CUTO37_DMSO", names(data_hg38))

CUTO33_crizo <- grep("CUTO33_250 nM crizotinib", names(data_hg38))
CUTO33_DMSO <- grep("CUTO33_DMSO", names(data_hg38))

CUTO28_crizo <- grep("CUTO28_250 nM crizotinib", names(data_hg38))
CUTO28_DMSO <- grep("CUTO28_DMSO", names(data_hg38))

CUTO27_crizo <- grep("CUTO27_250 nM crizotinib", names(data_hg38))
CUTO27_DMSO <- grep("CUTO27_DMSO", names(data_hg38))

CUTO23_crizo <- grep("CUTO23_250 nM crizotinib", names(data_hg38))
CUTO23_DMSO <- grep("CUTO23_DMSO", names(data_hg38))
```

Now we can start with DESEQ, finally.

```{r deseq-analysis-38}
dds <- DESeqDataSetFromMatrix(countData = data_hg38,
                              colData = col_data,
...
df <- as.data.frame(colData(dds)[,c("Assay.Type", "treatment")])
...
```{r gsea_38_data_prep}
hg_38_gene_list <- res$log2FoldChange
names(hg_38_gene_list) <- rownames(res)
hg_38_gene_list <- hg_38_gene_list[!is.na(hg_38_gene_list)]
hg_38_gene_list <- sort(hg_38_gene_list, decreasing = T)
head(hg_38_gene_list)
```

```{r, warning=FALSE}
entrezid_h38 <- bitr(names(hg_38_gene_list),
                fromType = "SYMBOL",
                toType = "ENTREZID",
                OrgDb = org.Hs.eg.db)

gene_list_h38 <- hg_38_gene_list[entrezid_h38$SYMBOL]
names(hg_38_gene_list) <- entrezid_h38$ENTREZID

hg_38_gene_list <- hg_38_gene_list[!is.na(names(hg_38_gene_list))]
hg_38_gene_list <- hg_38_gene_list[!duplicated(names(hg_38_gene_list))]


hg_38_gene_list <- sort(hg_38_gene_list, decreasing = TRUE)
```


```{r gsea_38}
gse_hg38 <- gseGO(
  geneList = hg_38_gene_list,
  OrgDb = org.Hs.eg.db,
  keyType = "ENTREZID",
  ont = "BP",
  pvalueCutoff = 0.05,
...
#| fig-height: 10
#| fig-width: 8
dotplot(gse_hg38, showCategory=10, split=".sign") + facet_grid(.~.sign)
```
...
```{r ridgeplot_38, warning=FALSE}
#| fig-height: 10
#| fig-width: 8
ridgeplot(gse_hg38) + labs(x = "Enrichment distribution")
...
#| fig-height: 8
#| fig-width: 8
gseaplot(gse_hg38, by = "all", title = gse_hg38$Description[1], geneSetID = 1)
```
...
data_hg19 <- read.csv2(paste0(config$counts_dir, "/hg19_counts.csv"), sep = ",", row.names = 1)
...
This is the count data for the GSE23 experiment, mapped to the hg19 human genome

```{r}
data_hg19 <- data_hg19[, rownames(col_data)]
all(rownames(col_data) == colnames(data_hg19))
```
...
```{r}
for (i in 1:nrow(col_data)) {
  idx <- grep(col_data$Run[i], names(data_hg19))
  names(data_hg19)[idx] <- col_data$condition[i]
}
```
...
```{r}
HCC78_crizo <- grep("HCC78_250 nM crizotinib", names(data_hg19))
HCC78_DMSO <- grep("HCC78_DMSO", names(data_hg19))

CUTO38_crizo <- grep("CUTO38_250 nM crizotinib", names(data_hg19))
CUTO38_DMSO <- grep("CUTO38_DMSO", names(data_hg19))

CUTO37_crizo <- grep("CUTO37_250 nM crizotinib", names(data_hg19))
CUTO37_DMSO <- grep("CUTO37_DMSO", names(data_hg19))

CUTO33_crizo <- grep("CUTO33_250 nM crizotinib", names(data_hg19))
CUTO33_DMSO <- grep("CUTO33_DMSO", names(data_hg19))

CUTO28_crizo <- grep("CUTO28_250 nM crizotinib", names(data_hg19))
CUTO28_DMSO <- grep("CUTO28_DMSO", names(data_hg19))

CUTO27_crizo <- grep("CUTO27_250 nM crizotinib", names(data_hg19))
CUTO27_DMSO <- grep("CUTO27_DMSO", names(data_hg19))

CUTO23_crizo <- grep("CUTO23_250 nM crizotinib", names(data_hg19))
CUTO23_DMSO <- grep("CUTO23_DMSO", names(data_hg19))
```

Now we can start with DESEQ, finally.

```{r deseq-analysis-19}
dds_hg19 <- DESeqDataSetFromMatrix(countData = data_hg19,
                              colData = col_data,
                              design = ~ Assay.Type + treatment)
dds_hg19 <- dds_hg19[rowSums(counts(dds_hg19)) > 10, ]
```

```{r}

dds_hg19 <- DESeq(dds_hg19, parallel = T)
```

```{r}
res_hg19 <- results(dds_hg19, contrast=c("treatment", "250 nM crizotinib", "DMSO"))

summary(res_hg19)
```

We see here that there are 7181 genes upregulated, and 4756 downregulated when the fasta files are mapped to the hg19 genome. 

```{r}
vsd_hg19 <- vst(dds_hg19, blind = F)
pcaData_hg19 <- plotPCA(vsd_hg19, intgroup= c("Assay.Type", "treatment"), returnData=T)

```

```{r}
percentVar_hg19 <- round(100 * attr(pcaData_hg19, "percentVar"))

ggplot(pcaData_hg19, aes(PC1, PC2, color = Assay.Type, shape = treatment)) +
  geom_point(size = 3) +
  xlab(paste0("PC1: ", percentVar_hg19[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar_hg19[2], "% variance"))

```

```{r}
resLFC_hg19 <- lfcShrink(dds_hg19, coef = tail(resultsNames(dds_hg19), n=1), type = "apeglm")
deseq.volcano(resLFC_hg19, "Treatment vs DMSO", v1 = "DMSO", v2 = "250 nm crizotinib")
```