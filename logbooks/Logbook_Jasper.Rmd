...
```{r, results='hide', message=FALSE}
dds_hg38 <- DESeqDataSetFromMatrix(countData = count_data_hg38[,desseq_for_CD74_ROS1], 
                              colData = coldata[desseq_for_CD74_ROS1,], 
                              design = ~ treatment)
...
```
```{r}
rld_hg19 <- rlog(dds_hg19_CD74, blind=T)
```
...
sampleDists <- dist(t(assay(rld_hg19)))
...
```{r}
sampleDists <- dist(t(assay(rld_hg38)))
sampleDistMatrix <- as.matrix(sampleDists)
